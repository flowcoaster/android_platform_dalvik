...
#ifdef WITH_TAINT_TRACKING
    //tweaking VM
    //gDvm.heapMaximumSize = 512 * 1024 * 1024;
    //ALOGD("tweaked dvm max heap Size to %d KB", gDvm.heapMaximumSize/1024);
#endif
        ...
        bool ignoreUnrecognized, JNIEnv* pEnv)
{
    ...
    sprintf(cpid, "%d %f", pid, (1000.0 * res.tv_sec + (double)(res.tv_nsec / 1000000)));
    const char* part2 = " &";
     ...
     */
    ALOGD("initializing components: gDvm.heapMaximumSize=%dKB", gDvm.heapMaximumSize/1024);
     ...
     */
    ALOGD("Now running initZygote/initAfterZygote...");
    if (gDvm.zygote) {
        if (!initZygote()) {
...
/*#ifdef WITH_TAINT_TRACKING
    ALOGD("-> initDispatcher(/system/lib/dispatcher.so)");
    if (!initDispatcher("/system/lib/dispatcher.so")) {
    	ALOGE("Loading dispatcher failed. Continuing without native tracking...");
    	//return false;
    }
 ...
 */
#ifdef WITH_TAINT_TRACKING
bool dvmInitAfterZygote(bool disableNativeTainting)
#else
bool dvmInitAfterZygote()
#endif
{
    ALOGD("dvmInitAfterZygote()");
#ifdef WITH_TAINT_TRACKING
    ...
    ALOGD("-> initDispatcher(/system/lib/dispatcher.so)");
    if (!initDispatcher("/system/lib/dispatcher.so", false)) {
    	ALOGE("Loading dispatcher failed. Continuing without native tracking...");
    	//return false;
    }
    //ALOGD(" <- initDispatcher()");
    ...
    ALOGD("thread-start heap=%d quit=%d jdwp=%d total=%d usec",
...
{
    ALOGD("dvmPrepForDexOpt(bootClassPath=%s); gDvm.heapMaximumSize=%dKB", bootClassPath, gDvm.heapMaximumSize/1024);
