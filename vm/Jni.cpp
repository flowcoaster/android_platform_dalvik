...
void dvmTrapTaintUseJNIBridge(Method* method, void* func) {
    ALOGD("-> dvmTrapTaintUseJNIBridge()");
    method->shouldTrace = shouldTrace(method);
    ...
    DalvikBridgeFunc bridge = dvmTrapTaintCallJNIMethod;
    dvmSetNativeFunc(method, bridge, (const u2*) func);
    ALOGD(" <- dvmTrapTaintUseJNIBridge()");
}
 ...
 */
void dvmCallJNIMethod(u4* args, JValue* pResult, const Method* method, Thread* self) {
  ...
  if(strstr(method->name, "HelloJni") != NULL)
    ...
    /*for (int i=tStart; i<=tEnd; i++) if (args[i] != 0) {
    	ALOGD("dvmCallJNIMethod(method %s:%s(%s)): taint for args[%d] = %d",
    		method->clazz->descriptor, method->name, method->shorty, i, args[i]);
    }*/
...
#ifdef WITH_TAINT_TRACKING
    dvmTaintPropJniMethod(oldArgs, pResult, method, 0);
    free(oldArgs);
#endif /*WITH_TAINT_TRACKING*/
...
#ifdef WITH_TAINT_TRACKING
void dvmTrapTaintCallJNIMethod(u4* args, JValue* pResult, const Method* method, Thread* self) {
  ALOGD("dvmTrapTaintCallJNIMethod for method %s", method->name);
    ...
    //for (int i=tStart; i<=tEnd && !tainted; i++) {
    	//ALOGD("taint for args[%d](%d) = %08x", i, args[i-nArgs-1], args[i]);
    ...
    /*} else {
	ALOGD("continuing normal execution without native taint tracking: dvmPlatformInvoke(env=%p, staticMethodClass=%p, argInfo=%d, argc=%d, modArgs=%p, shorty=%s, insns=%p, pResult=%p",
		env, staticMethodClass, method->jniArgInfo, method->insSize, modArgs, method->shorty, method->insns, pResult);
    	dvmPlatformInvoke(env,
            (ClassObject*) staticMethodClass,
            method->jniArgInfo, method->insSize, modArgs, method->shorty,
            (void*) method->insns, pResult);
    ...
    dvmTaintPropJniMethod(oldArgs, pResult, method, pResult2->taint);
    
    u4* rtaint = (u4*) &args[method->insSize];
    args[method->insSize] = pResult2->taint;
    //ALOGD("dvmTaintCallJniMethod: after taint propagation taint=%d", *rtaint);
    ...
    ALOGD("<-dvmTrapTaintCallJNIMethod in %fs", result);
}
#endif
...
#define SET_TYPE_FIELD(_ctype, _ctype2, _jname, _isref)                     \
    ...
    static void Set##_jname##TaintedField(JNIEnv* env, jobject jobj,        \
			 jfieldID fieldID, _ctype value, u4 taint) \
    {                                                                       \
        ALOGW("---- set a tainted Field");                                       \
        ScopedJniThreadState ts(env);                                       \
        Object* obj = dvmDecodeIndirectRef(ts.self(), jobj); \
        InstField* field = (InstField*) fieldID;                            \
        if (dvmIsVolatileField(field)) {                                    \
...
#define CALL_VIRTUAL(_ctype, _jname, _retfail, _retok, _isref)              \
    ...
    static _ctype Call##_jname##TaintedMethodA(JNIEnv* env, jobject jobj, u4 objTaint, \
...
static const struct JNINativeInterface gNativeInterface = {
    ...
    CallByteTaintedMethodA,
    CallCharTaintedMethodA,
    CallShortTaintedMethodA,
    CallIntTaintedMethodA,
