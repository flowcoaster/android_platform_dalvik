 ...
 * type definition.)
 */
void dvmResolveNativeMethod(u4* args, JValue* pResult,
    const Method* method, Thread* self)
{
	...
	if (!gDvm.disableNativeTainting) {
            dvmTrapTaintUseJNIBridge((Method*) method, func);
	} else {
#endif
...
#endif
	ALOGD("dvmResolveNative method now calling method->nativeFunc()");
...
#ifdef WITH_TAINT_TRACKING
    int32_t taintgrindHandle;			/* handle for taintgrind-tracked calls */
#endif
};
...
// hack: for efficiency, do not load system functions in tgwrapper
#ifdef WITH_TAINT_TRACKING
        ...
        char** detail)
{
    ...
    //if (verbose)
        ALOGD("dvmLoadNativeCode: Trying to load lib %s %p", pathName, classLoader);
...
#ifdef WITH_TAINT_TRACKING
    if (!blacklistedLibrary(pathName)) { // don't want to add libjavacore or other system stuff
	ALOGD("library not blacklisted, need to check if dispatcher is initialized");
	if (!gDvm.wrapperInitialized) {
	    ALOGD("Dispatcher not initialized yet, need to try again");
            initDispatcher("/system/lib/dispatcher.so", true);
	}
	int index = dvmAddTaintgrindLib(pathName);
	pNewEntry->taintgrindHandle = index;
    }
#endif
...
static int findMethodInLib(void* vlib, void* vmethod)
{
    ...
    if (func == NULL) {
	    ...
	    Method* meth2 = (Method*) vmethod;
	    meth2->tgFuncHandle = dvmAddTaintgrindFunc(mangleCMSig, pLib->taintgrindHandle);
        }
    } else {
        ALOGV("Found '%s' with dlsym", mangleCM);
	Method* meth2 = (Method*) vmethod;
	meth2->tgFuncHandle = dvmAddTaintgrindFunc(mangleCM, pLib->taintgrindHandle);
    }
...
void trapForTainting(const u4* args, JValue* pResult,
    const Method* method, Thread* self) {
	ALOGD("oh no, its a trap!");
}
...
static void* lookupSharedLibMethod(const Method* method)
{
    ...
    void* func =  (void*) dvmHashForeach(gDvm.nativeLibs, findMethodInLib,
        (void*) method);
    //if (func != NULL) {
    //	return (DalvikBridgeFunc*) (trapForTainting);
    //} else {
    	return func;
    //}
}
