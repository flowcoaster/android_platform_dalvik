    ...
    bool fromJni, JValue* pResult, va_list args)
{
    ...
    // ALOGD("---> dvmCallMethodV %s", method->name);
...
// DS20160705
// objTaint does not make sense as the single fields of the object could be tainted
// numArgs does not make sense as the method contains the shorty
...
#ifdef WITH_TAINT_TRACKING
            if (nativeTarget) {
              // TODO: call to native target from wrapper?
            	ins += 2;
            } else { /* adjust for taint tag interleaving */
            	ins[2] = ins[1];
            	ins[1] = taints[taintOffset++];
            	ins[3] = taints[taintOffset++];
            	ins += 4;
            }
            slot_cnt += 2;
#else
...
#ifdef WITH_TAINT_TRACKING
    	dvmInterpret(self, method, pResult, resultTaint);
#else
        dvmInterpret(self, method, pResult);
#endif
    }
