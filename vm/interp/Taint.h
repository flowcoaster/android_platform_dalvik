...
inline const char* taintToString(u4 taint) {
	return 	(char*)taint
			+   (taint&TAINT_LOCATION)?" location":""
			+	(taint&TAINT_CONTACTS)?" contacts":""
			+	(taint&TAINT_MIC)?" microphone":""
			+	(taint&TAINT_PHONE_NUMBER)?" phone number":""
			+	(taint&TAINT_LOCATION_GPS)?" GPS":""
			+	(taint&TAINT_LOCATION_NET)?" location(net)":""
			+	(taint&TAINT_LOCATION_LAST)?" location(last)":""
			+	(taint&TAINT_CAMERA)?" camera":""
			+	(taint&TAINT_ACCELEROMETER)?" accelerometer":""
			+	(taint&TAINT_SMS)?" sms":""
			+	(taint&TAINT_IMEI)?" imei":""
			+	(taint&TAINT_IMSI)?" imsi":""
			+	(taint&TAINT_ICCID)?" iccid":""
			+	(taint&TAINT_DEVICE_SN)?" serial-number":""
			+	(taint&TAINT_ACCOUNT)?" account-info":""
			+	(taint&TAINT_HISTORY)?" history":"";
}
